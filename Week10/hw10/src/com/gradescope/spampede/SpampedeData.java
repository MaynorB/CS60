...
class SpampedeData {
	...
	public void moveSnakeForward(BoardCell cell) {
		BoardCell headCell = getSnakeHead();
		BoardCell tailCell = getSnakeTail();
		if(cell.isSpam()){
	...
	protected BoardCell getNorthNeighbor(BoardCell cell) {
		int rowOfCell = cell.getRow();
		if (rowOfCell > 0){
			return getCell(rowOfCell - 1, cell.getColumn());
		}
		return null;
	}
	...
	protected BoardCell getEastNeighbor(BoardCell cell) {
		int colOfCell = cell.getColumn();
		if (colOfCell > 0){
			return getCell(cell.getRow(), colOfCell + 1);
		}
		return null; 
	}
  ...
  public void reverseSnake() {
	...
	BoardCell oldHead = getSnakeHead();
	oldHead.becomeBody();
    // Step 2: reverse the body parts
	LinkedList<BoardCell> temp = new LinkedList<BoardCell>();
	for(int i = snakeCells.size() - 1; i > 0; i--){
		temp.add(snakeCells.get(i));
	}
	snakeCells = temp;
	//Head is last thing
	BoardCell neckCell = this.snakeCells.get(1);
	BoardCell newHead = this.snakeCells.getFirst();
    // Step 3: relabel the head
	newHead.becomeHead();
    // Step 4: calculate the new direction after reversing!
	if(getNorthNeighbor().equals(neckCell) && getSouthNeighbor().isOpen()){
		setDirectionSouth();
	}
	else if(getSouthNeighbor().equals(neckCell) && getNorthNeighbor().isOpen()){
		setDirectionNorth();
	}
	else if(getWestNeighbor().equals(neckCell) && getEastNeighbor().isOpen()){
		setDirectionEast();
	}
	else if(getEastNeighbor().equals(neckCell) && getWestNeighbor().isOpen()){
		setDirectionWest();
	}
