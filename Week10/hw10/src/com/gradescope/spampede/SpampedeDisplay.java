...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		int horiOffSet = this.width / Preferences.CELL_SIZE - Preferences.NUM_CELLS_WIDE / Preferences.CELL_SIZE;
		int vertOffSet = this.width / Preferences.CELL_SIZE;
		for (int col = 0; col < this.theData.getNumColumns(); col++){
			for (int row = 0; row < this.theData.getNumRows(); row++){
				drawSquare(col * Preferences.CELL_SIZE + horiOffSet, row * Preferences.CELL_SIZE + vertOffSet, this.theData.getCellColor(row, col));
		}
	}
